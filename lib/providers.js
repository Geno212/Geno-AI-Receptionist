...
async function transcribe(audioBuffer, { filename = "audio.webm", language = "ar", timeoutMs = 20000 } = {}) {
  ...
  const mime = filename.endsWith(".ogg") ? "audio/ogg"
    : filename.endsWith(".m4a") || filename.endsWith(".mp4") ? "audio/mp4"
    : filename.endsWith(".mp3") ? "audio/mpeg"
    : filename.endsWith(".wav") ? "audio/wav"
    : "audio/webm";
  // Filename + mime matter: Groq picks the decoder from the extension and rejects
  // incomplete/corrupt containers with "could not process file".
  form.append("file", new Blob([audioBuffer], { type: mime }), filename);
  form.append("model", model);
  // Always verbose_json so we get language + segment no_speech_prob for hallucination gating.
  ...
  try {
    ...
    let noSpeechProb = null;
    let avgLogprob = null;
    if (segments.length) {
      noSpeechProb =
        segments.reduce((s, seg) => s + (Number(seg.no_speech_prob) || 0), 0) / segments.length;
      avgLogprob =
        segments.reduce((s, seg) => s + (Number(seg.avg_logprob) || 0), 0) / segments.length;
    }
    return {
      text: (data?.text || "").trim(),
      model,
